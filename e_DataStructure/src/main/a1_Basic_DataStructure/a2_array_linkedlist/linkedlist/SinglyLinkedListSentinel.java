...
public class SinglyLinkedListSentinel implements Iterable {
    ...
    // 赋值头指针为一个哨兵节点，该节点随意赋值(不用该值)。
    // 哨兵节点的添加使链表永不为空，可以使之后的步骤取消链表为空的判断，简化代码
    private Node head = new Node(296, null); // 头指针
     ...
     * @param node 链表节点
     */
    public SinglyLinkedListSentinel(Node node) {
        head.next = node;
    }
    ...
    public void addFirst(int value) {
         ...
         */
        // 注意加入哨兵后改为head.next，或者直接调用add(0,value)方法也可
    ...
    public void add(int index, int value) {
        // 若index == 0相当于向哨兵后插入节点，此时index - 1 = -1也可以找到指针(该指针即为哨兵指针)，因此不用单独判断
            /*if(index == 0) {
                addFirst(value);
                return;
            }*/
        ...
        // 不合法索引处理
        if(prev == null) {
            // 若一个类中包含多个相同异常，可以实现该异常方法(右键生成实现方法)直接调用抛出即可！
    ...
    public int removeLast() {
        ...
        // 链表仅有一个哨兵节点时
        if (head.next == null) {
            throw new IllegalArgumentException("A linkedlist has only sentinel nodes");
        }
        // 链表除哨兵有至少两个节点时
        Node p = head.next;
        while (p.next != null) {
    ...
    public int remove(int index) {
        ...
        /*
            图示：
                head哨兵             prev         removed待删除节点  待删除节点的下一个节点
                   |                  |                  |                  |
             ------ ------      ------ -------     ------ ------      ------ ------
    ...
    public Node removeIdenticalValue2(Node node) {
        ...
        head.next = removeIdenticalValue2_Test(node);
        return head.next;
    }
    ...
    public Node removeIdenticalAllValue(Node node) {
        ...
        head.next = s.next; // 该方法无法删除原链表的首个相同节点
        return head.next;
    }
    ...
    public void operation(Consumer<Integer> consumer) {
        // 加入哨兵节点后，如下所有的遍历操作都要从哨兵之后开始，而非头结点之后了
        Node p = head.next;
        while (p != null) {
    ...
    public Node insertionSort(Node node) {
        ...
        head.next = hasLinkedList(aux.next);
        return aux.next;
    }

-- EXPLAIN的使用
	...
	-- 2. id：在一个大的查询语句中每个SELECT关键字都对应一个唯一的id
		...
		EXPLAIN SELECT * FROM s1 WHERE key1 IN (SELECT key1 FROM s2) OR key3 = 'a';
			-- 有几个select就对应几个id，id跟表的多少无关

		-- -- -- -- -- -- 查询优化器可能对涉及子查询的查询语句进行重写,转变为多表查询的操作-- -- -- -- -- -- -- -- 
		EXPLAIN SELECT * FROM s1 WHERE key1 IN (SELECT key2 FROM s2 WHERE common_field = 'a');
			-- 优化器将子查询变为多表查询(多表连接后，仅一个select了，但是还是两个表)，id数量不变但化为一组
		...
		-- (1)id相同为一组，从上往下顺序执行
		-- (2)所有组中id值越大，优先级越高，越先执行
		-- (3)多个不同id表示一趟独立的查询，一趟次数越少越好



	-- 3. select_type：SELECT关键字对应的那个查询的类型,确定小查询在整个大查询中扮演了一个什么角色
		-- SIMPLE,PRIMARY,UNION,UNION RESULT,SUBQUERY,DEPENDENT SUBQUERY,DEPENDENT UNION,DERIVED,MATERIALIZED,
		...
		-- (1)对于包含`UNION`或者`UNION ALL`或者子查询的大查询来说，它是由几个小查询组成的，其中最左边的那个
		-- 查询的`select_type`值就是`PRIMARY`
		-- (2)对于包含`UNION`或者`UNION ALL`的大查询来说，它是由几个小查询组成的，其中除了最左边的那个小查询
		-- 以外，其余的小查询的`select_type`值就是`UNION`
		-- (3)`MySQL`选择使用临时表来完成`UNION`查询的去重工作，针对该临时表的查询的`select_type`就是`UNION RESULT`
		EXPLAIN SELECT * FROM s1 UNION SELECT * FROM s2;
		EXPLAIN SELECT * FROM s1 UNION ALL SELECT * FROM s2;
	...
	--  5. type：针对单表的访问方法
		...
		-- 当表中只有一条记录并且该表使用的存储引擎的统计数据是精确的，比如MyISAM、Memory(这些引擎会单独统计记录数量)，
		-- 那么对该表的访问方法就是`system`，性能最高
		CREATE TABLE t(i INT) ENGINE=MYISAM;
		INSERT INTO t VALUES(1);
		EXPLAIN SELECT * FROM t;
		-- 当表中有多条记录时，未设置索引则会遍历全表，访问方法为ALL，效率最低
		INSERT INTO t VALUES(1);
		EXPLAIN SELECT * FROM t;
		...
		EXPLAIN SELECT * FROM s1 WHERE key1 = 'a';
		EXPLAIN SELECT * FROM s1 WHERE key3 = 12; 
			-- key3为varchar类型，会用函数进行隐式转换，转换后不再使用对应索引了，因此访问方法变为ALL
	...
	-- 6. possible_keys和key：可能用到的索引和实际上使用的索引(非子集关系)
